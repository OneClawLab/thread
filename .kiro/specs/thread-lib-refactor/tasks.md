...
- [x] 1. 新建 `src/lib/types.ts`，定义 lib 层共享类型
...
- [x] 2. 新建 `src/lib/db.ts`，提取 SQLite 操作到 lib 层
  - [x] 2.1 将 `src/db/init.ts` 的 `openDb()` 和 `initSchema()` 复制到 `src/lib/db.ts`（保留原文件供 CLI 其他命令使用）
  - [x] 2.2 将 `insertEvent()`、`insertEventsBatch()` 从 `src/db/queries.ts` 复制到 `src/lib/db.ts`，参数类型改为 `ThreadEventInput`（来自 `src/lib/types.ts`）
  - [x] 2.3 新增 `peekEvents()` 函数（逻辑与现有 `popEvents()` 相同，返回类型改为 `ThreadEvent[]`）
  - _Requirements: 需求 5.1, 6.1, 7.1-7.4_

- [x] 3. 新建 `src/lib/event-log.ts`，迁移 JSONL 追加写入逻辑
...
- [x] 4. 新建 `src/lib/thread-store.ts`，实现 `ThreadStore` 类
  - [x] 4.1 实现 `ThreadStore` 类骨架：`readonly threadPath`、私有 `db`、私有 `closed` 标志、构造函数
  - [x] 4.2 实现 `push(event: ThreadEventInput): Promise<ThreadEvent>`
  ...
  - [x] 4.3 实现 `pushBatch(events: ThreadEventInput[]): Promise<ThreadEvent[]>`
    - 检查 `closed`
    - 空数组直接返回 `[]`
  ...
  - [x] 4.4 实现 `peek(opts: PeekOptions): Promise<ThreadEvent[]>`
    - 检查 `closed`
  ...
  - [x]* 4.6 编写 `ThreadStore` 单元测试（`vitest/unit/thread-store.test.ts`）
    - 测试 `push()` 写入并可通过 `peek()` 读回
    - 测试 `pushBatch()` 空数组返回 `[]`
    - 测试 `close()` 后调用 `push()` 抛出 `ThreadError(THREAD_CLOSED)`
    - _Requirements: 需求 5.1, 6.2, 8.2_

- [x] 5. 新建 `src/lib/thread-lib.ts`，实现 `ThreadLib` 类
  - [x] 5.1 实现私有 `_initDir(threadPath: string): void`
  ...
  - [x] 5.2 实现 `open(threadPath: string): Promise<ThreadStore>`
  ...
  - [x] 5.3 实现 `init(threadPath: string): Promise<ThreadStore>`
    - 检查 `events.db` 是否存在
  ...
  - [x] 5.4 实现 `exists(threadPath: string): Promise<boolean>`
    - 返回 `existsSync(path.join(threadPath, 'events.db'))`
    - _Requirements: 需求 3.1, 3.2, 3.3_
  - [x] 5.5 实现 `destroy(threadPath: string): Promise<void>`
    - 若目录不存在：直接返回（幂等）
    - 调用 `fs.rm(threadPath, { recursive: true, force: true })`
    - _Requirements: 需求 4.1, 4.2, 4.3_
  - [x]* 5.6 编写 `ThreadLib` 单元测试（`vitest/unit/thread-lib.test.ts`）
    - 测试 `open()` 对不存在路径自动初始化
    - 测试 `open()` 对已存在路径不破坏数据
...
- [x] 7. 检查点 — 确保所有测试通过，TypeScript 编译无错误
...
- [x] 9. 更新 `src/commands/init.ts`，改为调用 `ThreadLib.init()`
...
- [x] 10. 更新 `src/commands/push.ts`，改为调用 `ThreadStore.push()` / `pushBatch()`
...
- [x] 11. 更新 `src/commands/peek.ts`，改为调用 `ThreadStore.peek()`
...
- [x] 13. 更新 `tsup.config.ts` 为双 entry 构建
...
- [x] 16. 编写属性测试（`vitest/pbt/`）
  - [x]* 16.1 编写 `vitest/pbt/thread-lib.pbt.test.ts`
    ...
    - _Requirements: 需求 1-4_
  - [x]* 16.2 编写 `vitest/pbt/thread-store.pbt.test.ts`
