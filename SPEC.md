...
| `source` | TEXT | Event source identifier (the actor/component that produced the event) |
...
All commands require `--thread <path>` to specify the target thread directory. `push`, `pop`, `subscribe`, `info` support `--json` output; `unsubscribe` and `dispatch` do not.
